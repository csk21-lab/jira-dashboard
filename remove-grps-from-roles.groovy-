    ...
    projectRoles.each { role ->
        ...
        if (validGroupName) {
            ...
            if (groupsList) {
                ...
                log.warn("List of Invalid Groups Deleted from Role ->  (${role}) : GroupNames -> ${groupsList?.join(', ')}")
                projectRoleService.removeActorsFromProjectRole(groupsList, role, projectObject, ProjectRoleActor.GROUP_ROLE_ACTOR_TYPE, null)
